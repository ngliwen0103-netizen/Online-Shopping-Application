...
const BrandScreen = ({ route, navigation }: any) => {
  const { brand, fromMenu } = route.params;
  ...
  const loadBrandProducts = async () => {
    try {
      const productList = await getProductsByBrandId(brand.brand_id);
  ...
  const handleApplySort = () => {
    ...
    switch (selectedSort) {
      ...
      case 'name_desc':
        sortedList.sort((a, b) =>
          (b.product_name || b.name).localeCompare(a.product_name || a.name)
        );
        break;
      ...
      <View style={styles.stickyHeader}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.title}>
            {brand.brand_name?.toUpperCase() || 'BRAND'}
          </Text>
